...
// Store test pool reference - must be set before Pool is imported
let testPool: any;
...
vi.mock('pg', async () => {
  ...
  return {
    ...
    Pool: vi.fn().mockImplementation(() => {
      if (!testPool) {
        throw new Error('testPool not initialized - ensure setupTestDatabase runs first');
      }
      return testPool;
    }),
  };
...
describe('Admin Vanity Metrics API', () => {
  ...
  beforeAll(async () => {
    ...
    // Now import the route handler - it will use the mocked Pool
  ...
  describe('Authentication', () => {
    ...
    it('should accept valid admin token', async () => {
      await createTestUser(testDb.pool, { email: 'user1@test.com' });
